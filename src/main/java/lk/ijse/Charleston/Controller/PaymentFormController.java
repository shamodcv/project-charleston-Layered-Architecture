...
public class PaymentFormController {
   ...
   private Label lblOrderAm;
    ...
    PlaceOrderBo placeOrderBo = BoFactory.getBoFactory().getBO(BoFactory.BOTypes.PLACEORDER);
    ...
    private void getAll() {
        try {
            ...
            List<payment> paymentList = paymentBo.getAll();

            for (payment payment : paymentList) {
                obList.add(new PaymentTM(
                        payment.getPayID(),
                        payment.getGuest_ID(),
                        payment.getGuest_Name(),
                        payment.getRes_ID(),
                        payment.getRoom_ID(),
                        payment.getCheck_in_Date(),
                        payment.getCheck_out_Date(),
                        payment.getOrders_Ammount(),
    ...
    public void btnAddOnAction(ActionEvent actionEvent) throws ClassNotFoundException {
        ...
        try {
            boolean isSaved = paymentBo.save(new Payment(paymentId,guestId,guestName,reservationId,roomId,checkinDate,checkoutDate,ordersAm,totalPrice));
            if (isSaved) {
    ...
    public void btnUpdateOnAction(ActionEvent actionEvent) throws ClassNotFoundException {
        ...
        try {
            boolean isUpdated = paymentBo.update(new Payment(paymentId,guestId,guestName,reservationId,roomId,checkinDate,checkoutDate,ordersAm,totalPrice));
            if(isUpdated){
    ...
    public Double getOrderAmmount(String resId) throws ClassNotFoundException {
        Double orderAmmount = 0.0;
        try {
            orderAmmount = placeOrderBo.getOrderAm(resId);
