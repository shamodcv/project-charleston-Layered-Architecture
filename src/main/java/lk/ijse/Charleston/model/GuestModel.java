...
public class GuestModel {
    ...
    public static boolean save(Guest guest) throws SQLException {
        String sql = "INSERT INTO Guest(Guest_ID_Type, Guest_ID, Guest_Name, Guest_Contact_No, Guest_Country, Guest_Email) VALUES(?, ?, ?, ?, ?, ?)";
        PreparedStatement pstm = DBConnection.getInstance().getConnection().prepareStatement(sql);
    ...
    public static boolean update(Guest guest) throws SQLException {
        String sql = "UPDATE Guest SET Guest_ID_type = ?, Guest_Name = ?, Guest_Contact_No = ?, Guest_Country = ?, Guest_Email = ? WHERE Guest_ID = ?";
        PreparedStatement pstm = DBConnection.getInstance().getConnection().prepareStatement(sql);

        pstm.setString(1, guest.getIdType());
        pstm.setString(2, guest.getName());
        pstm.setString(3, guest.getContactNo());
        pstm.setString(4, guest.getEmail());
        pstm.setString(5, guest.getId());
